...
namespace CinemaDrape
{
    ...
    public partial class MainForm : Form
    {
        ...
        public bool status;
        private FocusControl focusControlInstance;
        public int MainClassstatus { get; set; }
        public event EventHandler MainClassVariableChanged;
        ...
        public MainForm()
        {
            ...
            focusControlInstance = new FocusControl();

            // Subscribe to the event
            focusControlInstance.ShowPasswordFormRequested += FocusControl_ShowPasswordFormRequested;

        }


        private bool mainClassVariable;

        public bool MainClassVariable
        {
            get { return mainClassVariable; }
            set
            {
                mainClassVariable = status;
                OnMainClassVariableChanged(EventArgs.Empty);
            }
        }

        protected virtual void OnMainClassVariableChanged(EventArgs e)
        {
            MainClassVariableChanged?.Invoke(this, e);
        }


        private void FocusControl_ShowPasswordFormRequested(object sender, EventArgs e)
        {
            // Handle the event, e.g., show the password form
        ...
        private void EventMainFormLoad(object sender, EventArgs e)
        {
            try
            {
                ...
                // Create the quick start form
                this.passwordForm = Password_Form.Create(this);

                // Create the quick start form
                this.passwordlogin = Password_Login.Create(this);
        ...
        private void ShowPasswordLoginForm()
        {
            Password_Login passwordLoginForm = Password_Login.Create(this);

            // Show the password login form as a dialog
            DialogResult result = passwordLoginForm.ShowDialog();
        ...
        public void SetLoginStatus(bool isLoggedIn)
        {
            if (isLoggedIn)
            {
                // Implement actions to take when the user is logged in
                // MessageBox.Show("Login successful!");
                status = true;
            }
            else
            {
                // Implement actions to take when the login fails
